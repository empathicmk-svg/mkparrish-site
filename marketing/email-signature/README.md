...
| `mk-parrish-signature.jpg` | The card as a flat image — for anywhere an HTML signature isn't accepted (an image-only upload field, a text, a quick look on a phone). |
...
mbusa.com and mercedes-benz.com set headlines in **MB Corpo A Title** — a serif —
over body copy in **MB Corpo S Text**, a grotesque. Both are licensed through the
Mercedes-Benz Font Service, so neither can be served from this repo, and that
would not help anyway: Gmail, Outlook and Yahoo all drop `@font-face`, so no
email signature anywhere loads a webfont. Whatever is installed on the reading
machine is what renders.

So the stacks name the real MB families first and fall through: Georgia for the
serif name (the one serif installed on every Mac and Windows machine) and
Helvetica/Arial for everything else. Any machine with the dealer brand kit
installed — likely including the showroom's own — renders true MB Corpo.
...
Outlook desktop without the brand kit. `mso-ascii-font-family` and its two
siblings pin Word to Georgia for the name and Arial for the rest; every other
client ignores them.
...
The Mercedes star sits at the top in silver, as the store's marque. MK's heart —
the same one that is the cursor on mkparrish.com, `public/cursor-heart.svg` —
sits in the flourish between the store line and her name, flanked by two hairlines.

The site's heart is petal pink outlined in near-black, which disappears against
this card, so `public/email/mk-heart.png` keeps the fill and drops the outline.
Both marks are generated by the build script.

## The QR code

The card carries a scan-to-review code under the button, generated from
`DATA.review.href` by the same build — change the link and the code follows. It
sits on a white tile because a QR has to be dark-on-light: inverted codes defeat
a good share of scanners.
...
`mk-heart.png` and `review-qr.png` that way and let the editor rewrite each `src` — the signature then works before
...
- **Garamond italic → the Mercedes-Benz register.** Garamond is not installed on
  Windows and was falling back to Georgia anyway. The name is now set in the MB
  headline serif, Georgia where MB Corpo is absent. See *The font* above.
